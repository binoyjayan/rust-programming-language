// A Multithreaded version of Mandelbrot plotter
use std::env;
use num::Complex;
use std::fs::File;
use std::str::FromStr;
...
fn main() {
    ...
    if args.len() != 5 {
        eprintln!("Usage: {} <file> <pixels> <upperl-left> <lower-right", args[0]);
    ...
    let rows_per_band = bounds.1 / threads + 1;
    {
        ...
        // crossbeam::scope calls the closure with 'spawner' as an argument
        // that can be used to spawn threads from within the closure
...
/// Try to determine if `c` is in the mandelbrot set using atmost limit iterations
/// 
/// If `c` is not a member, return `Some(i)` where `i` is the number of iteratrions
...
/// designating the are our image covers.
