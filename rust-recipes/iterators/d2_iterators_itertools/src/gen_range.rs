...
#[cfg(test)]
mod tests {
    // bring IterCombi into scope
    use super::*;
    #[test]
    fn gen_test1() {
        let mut m = 0;
        let it = GenRangeIterator::new(5, 12, 3);
        for s in it {
            m += s;
        }
        assert_eq!(m, 5 + 8 + 11, "Test 1 -integers");
    }

    #[test]
    fn filter() {
        let v:i32 = GenRangeIterator::new(3, 13, 3).filter(|x| x % 2 == 0).sum();
        // 6, 12 is the only two even numbers that pass this filter
        assert_eq!(v, 6 + 12, "Test 2 - filters");
    }
}
